...
public final class Swarm {
    ...
    private final Map<Hash, Torrent> torrents = new LinkedHashMap<>(32);
    ...
    public @Nullable Torrent getTorrent(@Nullable Hash infoHash) {
        if (infoHash == null) {
            return null;
        }
        synchronized (lock) {
    ...
    public @NotNull Torrent addTorrent(@NotNull Info info) {
        Objects.requireNonNull(info, "Argument 'info'");
        synchronized (lock) {
            return torrents.computeIfAbsent(info.getHash(), (hash) -> new Torrent(this, info));
        }
    }
